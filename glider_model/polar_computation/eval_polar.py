...
def load_polar(polar_file):
    ...
    assert len(speeds) == len(
        sink_rates
    ), "Speeds and sink rates must have the same length."
...
def plot_polar(speeds, sink_rates, V_star, s_min, k_v):
    ...
    modelled_sink_rates = get_modelled_sink_rate_zero_bank(
        speed_range, V_star, s_min, k_v
    )
    plt.plot(
        speed_range, modelled_sink_rates, "-", label="Fitted Model", color="orange"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--polar-file", type=str, required=True, help="Path to the polar JSON file."
    )
    parser.add_argument(
        "--plot", action="store_true", help="Whether to plot the polar curve."
    )
