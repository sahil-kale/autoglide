...
class GliderModelParams:
    def __init__(self, V_star, s_min, k_v, alpha_n, initial_altitude, roll_tau, vel_tau):
...
class GliderOpenLoopKinematicModel:
    ...
    def step(self, dt, control: GliderKinematicModelControl, disturbance: GliderKinematicModelDisturbance):
        ...
        psi_dot = GRAVITY_M_PER_S_SQ / self.V * np.tan(self.phi) # turn-coordinated kinematic model
    ...
    def get_sink_rate(self, V, phi, params: GliderModelParams):
        s0 = params.s_min + params.k_v * (V - params.V_star) ** 2 # Sink rate at zero bank angle
