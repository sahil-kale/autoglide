...
def animate_single_window(
    t, phi, theta, psi, p, q, r, beta, ua, ue, ur, x, y, z,
    window_s=10.0, speed=1.0, show_rates=False, show_beta=False, save_path=None
):
    dt = float(t[1]-t[0])
    ...
    if show_beta:
        ...
        ax_beta.legend(loc="upper right"); ax_beta.grid(True)
    ...
    def init():
        ...
        for ax in [ax_act, ax_ang] + ([ax_rate] if ax_rate is not None else []) + ([ax_beta] if ax_beta is not None else []):
            if ax is not None:
...
def main():
    ...
    animate_single_window(
        t, phi, theta, psi, p, q, r, beta, ua, ue, ur, x, y, z,
        window_s=args.window_s, speed=args.speed,
        show_rates=args.show_rates, show_beta=args.show_beta,
        save_path=args.save
    )
